                ...
                if (GameManager.Instance.CurrentScene == GroundScene.Instance)
                {
                    for (int ii = 0; ii < entry.GroundUseActions.Count; ii++)
                    {
                        ...
                        if (groundEvent.Visible)
                        {
                            ...
                            choices.Add(new MenuTextChoice(actionName, action, !invItem.Cursed && groundEvent.Enabled,
                                invItem.Cursed | !groundEvent.Enabled ? Color.Red : Color.White));
                        }
                    }
                }
